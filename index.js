...
function get20s(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    const lifespan = array[i].years.split(' ');
    if (lifespan[0] >= 1900 && lifespan[2] <= 2000) {
      century_20.push(array[i].name);
    }
  }
  return century_20;
}
...
function lotsOfArt(array) {
  const hundred_plus_paintings = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100) {
      hundred_plus_paintings.push(array[i].name);
    }
  }
  return hundred_plus_paintings;
}
